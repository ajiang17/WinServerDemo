...
//    QSize logoSize;
//    logoSize.setWidth(image.width()*0.25);
//    logoSize.setHeight(image.height()*0.25);
...
int PrintFeatures::doPrint()
{
    ...
    s.width(), s.height());
    // 发送剩余打印数量
    m_LogoMap = m_LogoMap.scaled(237,237).scaled(64,64,Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
    painter.drawPixmap(32,32, m_LogoMap);
    painter.end();
